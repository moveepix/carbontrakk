...
class CarbonTrakkSupabaseUser extends BaseAuthUser {
  CarbonTrakkSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> carbonTrakkSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CarbonTrakkSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
